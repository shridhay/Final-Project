...
need to work out some package dependencies still
...
class MusicBeatAnalyzerNode(Node):
    ...
    def analyze_song(self, music_file):
        ...
        try:
            ...
            self.get_logger().info(f'Loaded audio: {len(y)} samples at {sr} Hz')
            ...
            energy_times = librosa.frames_to_time(np.arange(len(energy)), sr=sr)
            
            # Publish tempo
            tempo_msg = Float32()
            tempo_msg.data = float(tempo)
            self.tempo_pub.publish(tempo_msg)
            self.get_logger().info(f'Tempo: {tempo:.1f} BPM')
            
            # Publish beat times
            beat_msg = Float32MultiArray()
            beat_msg.data = beat_times.tolist()
            self.beat_times_pub.publish(beat_msg)
            self.get_logger().info(f'Found {len(beat_times)} beats')
            self.get_logger().info(f'Beat times (first 10): {beat_times[:10]}')
            
            # Publish energy
            energy_msg = Float32MultiArray()
            energy_msg.data = energy.tolist()
            self.energy_pub.publish(energy_msg)
            self.get_logger().info(f'Energy samples: {len(energy)}')
...
def main(args=None):
    ...
    try:
        # Spin briefly to ensure messages are published
...
https://github.com/mgonzs13/audio_common?tab=readme-ov-file#installation
